...
import { CATEGORIES, MARKET_BOOKS } from "@/lib/books/catalog";
  ...
  return (
    <div className="space-y-8">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
        <div>
          <h1 className="font-serif text-3xl font-medium tracking-tight">书城</h1>
          <p className="mt-1 text-fg-muted">发现好书，加入书架，随时开读</p>
        </div>
      ...
      <section className="rounded-[var(--radius-xl)] border border-border bg-bg-elevated p-4 sm:p-5">
        <div className="mb-3 flex items-center justify-between">
          <h2 className="text-sm font-medium text-fg-muted">热门在读</h2>
          <Badge variant="outline">{MARKET_BOOKS.length} 本上架</Badge>
        </div>
          ...
          {hot.map((b, i) => (
            ...
            >
              ...
              <div className="min-w-0">
                ...
                <p className="truncate text-xs text-fg-subtle">{b.author}</p>
              </div>
            </div>
          ))}
        </div>
          ...
          {(
            ...
            <button
              ...
              className={cn(
                ...
                view === k ? "bg-bg-subtle text-fg" : "text-fg-muted",
              )}
            >
              {label}
            </button>
      ...
      {books.length === 0 ? (
        <div className="rounded-[var(--radius-xl)] border border-dashed border-border py-16 text-center text-fg-muted">
          没有匹配的图书，试试其他关键词
        </div>
      ) : view === "grid" ? (
