...
import { n as ensureDbReady, r as getPglite } from "./db-BwYgqUFs.mjs";
...
//#region node_modules/.nitro/vite/services/ssr/assets/server-L6uBiSHQ.js
...
function deployHosts() {
	const hosts = /* @__PURE__ */ new Set();
	const push = (raw) => {
		if (!raw) return;
		const h = raw.replace(/^https?:\/\//, "").replace(/\/$/, "").trim();
		if (h) hosts.add(h);
	};
	push(env$1("VERCEL_URL"));
	push(env$1("VERCEL_PROJECT_PRODUCTION_URL"));
	push(env$1("VERCEL_BRANCH_URL"));
...
var deployHostList = deployHosts();
var deployOrigins = deployHostList.flatMap((host) => {
	if (host.startsWith("*.")) return [`https://${host}`, `http://${host}`];
	return [`https://${host}`];
});
